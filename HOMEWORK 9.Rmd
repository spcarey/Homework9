...
boost_fit_1 <- gbm(as.character(target) ~ ., data = AMZ_training, distribution = "bernoulli",cv.folds = 5 , n.trees = 5000, shrinkage = 0.1, interaction.depth = 3)
...
#obtain the best # of trees based on best test interation 
boost_fit_1_best_test <- gbm.perf(boost_fit_1, method = "test")

gbm.perf(boost_fit_1, method = )

#make prediction based on best Cross Validation number of trees and best test number of trees
boost_pred <- predict.gbm(boost_fit_1, newdata = dplyr::select(AMZ_test, -target),type = "response", n.trees = c(boost_fit_1_best_cv, boost_fit_1_best_test))
...
boost_pred$class_cv <- ifelse(boost_pred$`2949` >= 0.5, 1, 0)
boost_pred$class_test <- ifelse(boost_pred$`3739` >= 0.5, 1, 0)
...
#create table comparing test data to predictions using the best test interation # of trees
boost_table_test <- table(data.frame(boost_pred$class_test, AMZ_test$target))

#obtain test error rate based on best test iteration number of trees
boost_test_error_test <- sum(diag(boost_table_test)/sum(boost_table_test))

# create table with both error rates
boost_error_table <- c(cv=boost_test_error_cv, test=boost_test_error_test)

boost_error

#Based on the test erro rate the best # of trees is 3739. This gives us a 0.960 error rate. 
